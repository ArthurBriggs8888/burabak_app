...
import useGetUserAgreement from '../hooks/useGetUserAgreement'
...
const UserAgreement = ({navigation}) => {
    const [agreement, getUserAgreement] = useGetUserAgreement();
    const style = `
    ...
    useEffect(() => {
        getUserAgreement(getCurrentLanguage())
    }, [i18n.locale])
    ...
    return (
        <View style={[Styles.container, GlobalStyle.padding(20, 20, 20, 20)]}>
            ...
            { agreement !== '' && (<WebView style={[GlobalStyle.margin(20, 0, 0, 0)]} source={{html: `${style}${agreement}`}} />) }
            { agreement === '' && <ActivityIndicator size='large' style={{position: 'absolute', left: '50%', top: '50%'}} /> }
        </View>
    )
}
